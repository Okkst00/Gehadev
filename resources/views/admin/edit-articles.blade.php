...
<body>
    ...
    <div class="container bg-white my-5 rounded-5 shadow">
        <div class="row justify-content-center py-5">
            ...
            <a href="/admin/artikel" class="btn-back py-2 px-4"><i class="bi bi-arrow-left-circle-fill"></i> Kembali</a>
            <h2 class="text-center main-title pb-5">Edit Artikel -- {{ $posts->id }} </h2>
            <div class="text-center col-lg-10 col-md-10 col-11 mt-5">
                <form action="/admin/artikel/{{ $posts->id }}" method="post" enctype="multipart/form-data"> 
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        <textarea class="form-control mb-3" rows="2" name="slug" placeholder="Slug" id="slug" required >{{ $posts->slug }} </textarea>
                    </div>
                    <select class="form-select mb-3 w-100" name="category" id="category" required >
                        <option value="{{ $posts->category }}" selected>{{ $posts->category }}</option>
